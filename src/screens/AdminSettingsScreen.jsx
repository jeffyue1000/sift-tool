...
export default function AdminSettingsScreen() {
    ...
    const handleRejectOrPushQuotaSubmit = async (quota, type) => {
        try {
            const res = await axios.post(
                `http://localhost:3001/sessions/updateRejectOrPushQuota`,
                {
                    quota: quota,
    ...
    const handleRejectOrPushAdminRequired = async (checked, type) => {
        try {
            const res = await axios.post(
                `http://localhost:3001/sessions/updateRequireAdminPushOrReject`,
                {
                    checked: checked,
                    sessionID: sessionDetails.sessionID,
                    type: type,
    ...
    const handleUseRejectOrPush = async (checked, type) => {
        try {
            const res = await axios.post(
                `http://localhost:3001/sessions/updateUsePushOrReject`,
                {
                    checked: checked,
                    sessionID: sessionDetails.sessionID,
                    type: type,
            ...
            <div className="page-container">
                ...
                <div className="settings-container">
                    <div className="reject-container">
                        ...
                        {useReject && (
                            <QuotaSetting
                                className="quota-setting"
                                settingName="# of rejections required"
                    ...
                    <div className="push-container">
                        ...
                        {usePush && (
                            <QuotaSetting
                                className="quota-setting"
                                settingName="# of pushes required"
                                handleSubmit={handleRejectOrPushQuotaSubmit}
                                quota={pushQuota}
